...
const newClusterData = {
  ...
  "nodes": [{ "name": "k8s-control-plane-1", "status": "Ready" }],
  "deployments": [
  ...
  return (
    <Card className={`w-64 shadow-md ${data.isActive ? 'border-2 border-green-500 animate-pulse' : ''}`}>
      ...
      <CardContent className="p-3 text-xs">
        ...
        {data.isActive && <div className="text-center text-green-500 font-bold mt-2 text-xs">{t("podStatus.serving")}</div>}
      </CardContent>
      <Handle type="target" position={Position.Top} className="w-2 h-2" />
    </Card>
  );
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center"><Network className="mr-2" /> {t("overview.title")}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">{t("overview.k8sVersion")}</span>
                  <Badge variant="outline">{k8sData.serverVersion}</Badge>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">{t("overview.nodeStatus")}</span>
                  <div className='flex items-center gap-2'>
                    <Badge variant="default" className="bg-green-600 hover:bg-green-700">
                      {k8sData.nodes.filter(n => n.status === 'Ready').length} {t("overview.ready")}
                    </Badge>
                    <Badge variant="destructive">
                      {k8sData.nodes.filter(n => n.status !== 'Ready').length} {t("overview.unhealthy")}
                    </Badge>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="outline" size="sm" className="w-full">{t("overview.showNodes")}</Button>
                    </TooltipTrigger>
                    <TooltipContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center"><CircleDot className="mr-2 text-green-500" />{t("activePod.title")}</CardTitle>
                <CardDescription>{t("activePod.description")}</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center"><GitCommitHorizontal className="mr-2" /> {t("activeDeployment.title")}</CardTitle>
                <CardDescription>{t("activeDeployment.description")}</CardDescription>
              </CardHeader>
